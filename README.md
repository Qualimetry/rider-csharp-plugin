...
A JetBrains Rider plugin for the Qualimetry C# rule set. Once set up, Qualimetry findings appear in the editor in real time as you type. The analysis runs inside Rider's bundled Roslyn host; this plugin wires the Qualimetry analyzer into your solution and syncs the active rule set so those findings show up as you work.
...
- **Works on open** - when you open a solution that does not yet reference the analyzer, the plugin offers to add it. Accept once and findings start appearing as you type. You can turn this off under **Settings > Tools > Qualimetry C#**, or run the action below manually.
...
Analysis is performed by the Roslyn analyzer once the NuGet is referenced; this plugin provisions and configures it for you. Configure the SonarQube URL, token, and profile name under **Settings > Tools > Qualimetry C#**.
